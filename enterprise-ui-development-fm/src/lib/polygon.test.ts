...
describe("Polygon", () => {
  it("should throw an error if given less than 3 sides", () => {
    expect(() => new Polygon(2, 10)).toThrowError("must have three or more sides")
  })
...
`("$polygonType", ({ polygonType, sides, lengthOfSide, sumOfAngles, perimeter, area }) => {
  ...
  it<ContextWithPolygon>(`should have ${sides} sides`, async ({ polygon }) => {
    expect(polygon.sides).toBe(sides)
  })

  it<ContextWithPolygon>(`should have the correct type`, async ({ polygon }) => {
    expect(polygon.type).toBe(polygonType)
  })

  it<ContextWithPolygon>(`should have the correct sum of its angles`, async ({ polygon }) => {
    expect(polygon.sumOfAngles).toBe(sumOfAngles)
  })

  it<ContextWithPolygon>(`should have the correct perimeter`, async ({ polygon }) => {
    expect(polygon.perimeter).toBe(perimeter)
  })

  it<ContextWithPolygon>(`should have the correct area`, async ({ polygon }) => {
    expect(polygon.area).toBeCloseTo(area)
  })

  it<ContextWithPolygon>(`should generate JSON that matches the snapshot`, ({ polygon }) => {
