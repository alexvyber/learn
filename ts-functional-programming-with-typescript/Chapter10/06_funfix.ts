...
namespace funfix_demo_1 {
  ...
  const readFile = (filename: string) =>
    IO.of(() => fs.readFileSync(filename, "utf8"))
  const stdoutWrite = (data: string) => IO.of(() => process.stdout.write(data))

  const loudCat = argsIO.chain(readFile).map(R.toUpper).chain(stdoutWrite)
...
namespace funfix_demo_2 {
  ...
  const getTodos = IO.async<Either<Error, Todo[]>>((ec, cb) => {
    fetch("https://jsonplaceholder.typicode.com/todos")
      .then(response => {
        return response.json().then((json: Todo[]) => cb(Success(Right(json))))
      })
