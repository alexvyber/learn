...
app.register(
  async function pluginA(instance) {
    app.addHook("onRequest", function auth(request, reply, done) {
      done()
    })
    app.addHook("preHandler", function isFeline(request, reply, done) {
      done()
    })
...
app.addHook("preHandler", function isAnimal(request, reply, done) {
  done()
})
app.get("/", handler)
